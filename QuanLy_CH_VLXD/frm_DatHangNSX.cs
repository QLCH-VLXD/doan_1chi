    ...
    public partial class frm_DatHangNSX : UserControl
    {
        ...
        private void btn_Sua_Click(object sender, EventArgs e)
        {
            ...
            if (bLL_DatHangNSX.Sua_CTDHNSX(ct) == true)
            {
                ...
                // /////////////
                int sl = Convert.ToInt32(bLL_DatHangNSX.loasl(dataGridView_CTDatHangNSX.CurrentRow.Cells[0].Value.ToString()));
                int slc = Convert.ToInt32(bLL_NhapHang.load_SoLuong(dataGridView_CTDatHangNSX.CurrentRow.Cells[0].Value.ToString()));
                if (sl > slc)
                {
                    h.HOANTAT = false;
                }
        ...
        {
            if(dataGridView_CTDatHangNSX.CurrentRow!=null&& dataGridView_DatHangNSX.CurrentRow!=null)
            txt_MaPDNSX.Enabled = cbo_NhaSanXuat.Enabled = false;
            txt_MaPDNSX.Text = dataGridView_DatHangNSX.CurrentRow.Cells[0].Value.ToString();
            cbo_NhaSanXuat.Text = dataGridView_DatHangNSX.CurrentRow.Cells[2].Value.ToString();
        ...
        {
            if (dataGridView_CTDatHangNSX.CurrentRow != null)
            {
            ...
            {
                MessageBox.Show("Không được phép xóa phiếu đặt");
                return;
            }
        }
